...
def _client(ep: Endpoint):
    ...
    # The free endpoint stalls occasionally; a stalled call must fail and be
    # retried rather than hang a run. The runner is resumable either way.
    return OpenAI(base_url=ep.base_url, api_key=key, timeout=REQUEST_TIMEOUT_S, max_retries=3)
