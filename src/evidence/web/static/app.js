...
function renderCoverage(obligations, results, agreement, repeats) {
  ...
  return obligations.map((o) => {
    ...
    if (o.level === "does_not_cover") body = `<ul><li>${esc(o.reason)}</li></ul>`;
      ...
      const rows = o.check_basis.map((c) => `<tr><td><code class="inline">${esc(c.name)}</code></td><td style="white-space:nowrap">${resultTag(c)}</td><td class="wrap" style="color:var(--ink-2)"><b style="color:var(--ink)">${esc(c.ref || "")}</b> ${esc(c.tests || "")}</td></tr>`);
      if (o.judge) rows.push(`<tr><td><code class="inline">judge · ${esc(o.judge.name)}</code></td><td style="white-space:nowrap">${o.judge.result ? `<span class="tag">mean ${o.judge.result.mean_value}</span> reported` : `<span class="badge ${noRun ? "good" : ""}">${noRun ? "built" : "not run"}</span>`}</td><td class="wrap" style="color:var(--ink-2)"><b style="color:var(--ink)">${esc(o.judge.ref)}</b> ${esc(o.judge.tests)}</td></tr>`);
      if (o.metrics) for (const [k, m] of Object.entries(o.metrics)) {
        ...
        rows.push(`<tr><td><code class="inline">${esc(k)}</code></td><td style="white-space:nowrap">${worst && repeats > 1 ? `<span class="tag">lowest ${pct(worst.stable, worst.items)}</span>` : `<span class="badge">needs repeats</span>`}</td><td class="wrap" style="color:var(--ink-2)"><b style="color:var(--ink)">${esc(m.ref)}</b> ${esc(m.tests)}</td></tr>`);
      }
      if (o.process) rows.push(`<tr><td><code class="inline">lender's process</code></td><td style="white-space:nowrap">${results.__rules ? `<span class="badge ${results.__rules === "pass" ? "good" : results.__rules === "fail" ? "bad" : ""}">${esc(results.__rules)}</span>` : `<span class="badge">rule pack</span>`}</td><td class="wrap" style="color:var(--ink-2)"><b style="color:var(--ink)">${esc(o.process.ref)}</b> ${esc(o.process.tests)}</td></tr>`);
      body = req + (rows.length ? `<table class="grid-table" style="margin-top:6px"><thead><tr><th>test</th><th>result</th><th>what it tests, and why that is evidence</th></tr></thead><tbody>${rows.join("")}</tbody></table>` : `<p class="hint">No test declared.</p>`);
    }
