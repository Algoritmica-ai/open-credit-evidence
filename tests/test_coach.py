...
def test_asked_about_one_finding_the_coach_answers_about_that_one_only():
    seen = []

    def call(model, messages, **kw):
        seen.append(messages[-1]["content"])
        return _reply({"challenges": [{"finding": 1, "question": "Where is 36.5% above 40%?",
                                       "evidence": "36.55%"}],
                       "reply": "The figures give 36.55%, under the 40% limit."})

    s = coach.start("run", "about#r0", "super")
    first = [{"card_id": "c1", "action": "dispute", "reason": "wrong_figure"},
             {"card_id": "c2", "action": "dispute", "reason": "finding_wrong"}]
    r = coach.turn(s, evidence="E", cards=CARDS, verdicts=first, raised=[], message=None,
                   call=call, about="c1")
    assert r["about"] == "c1" and r["challenges"][0]["card_id"] == "c1"
    assert "asks about finding 1 only" in seen[0]
    # c2 changed without the coach saying anything about it: not the coach's doing
    final = [{"card_id": "c1", "action": "confirm"}, {"card_id": "c2", "action": "confirm"}]
    kept = coach.record(s, final, first)
    assert kept["asked_about"] == ["c1"] and kept["changed_after_coach"] == ["c1"]
    assert kept["conversation"][0]["about"] == "c1" and kept["version"] == "coach-v3"
    coach.close(s.id)
    assert coach.get(s.id) is None
