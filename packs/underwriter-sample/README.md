...
Twenty referred personal-loan applications with a known answer. This is what the
assistant under test is given, and what it is marked against.
...
| `regulatory_context.json` | The lender and product the assistant serves, with evidence references, for the jurisdiction rule pack (`evidence rules`). Example values from `regulations/IT`. | yes |
...
The rule for *which* facts are omission targets is: the top driver, any policy limit
breached, and any recent adverse item. It lives in `omission_targets()` in the builder
and is the materiality rule this pack version applies.
